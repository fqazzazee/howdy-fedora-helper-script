# Howdy Fedora Helper Script — Wiki

![Version](https://img.shields.io/badge/version-1.3.2-informational.svg)
![Fedora 40+](https://img.shields.io/badge/Fedora_40%2B-required-blue?logo=fedora&logoColor=white)
![Fedora 43](https://img.shields.io/badge/Fedora_43-tested-brightgreen?logo=fedora&logoColor=white)
![Fedora 44](https://img.shields.io/badge/Fedora_44-tested-brightgreen?logo=fedora&logoColor=white)
![Desktop](https://img.shields.io/badge/desktop-GNOME%20%2F%20GDM-success.svg?logo=gnome)

Welcome to the wiki for the Howdy Fedora Helper Script. This wiki provides supplementary information beyond the main documentation.

## Documentation

| Document | Description |
|----------|-------------|
| [README](../blob/main/README.md) | Project overview and quick start |
| [CHANGELOG](../blob/main/CHANGELOG.md) | Release notes and version history |
| [CONTRIBUTING](../blob/main/CONTRIBUTING.md) | How to contribute, code style, SELinux policy notes |

## Wiki Pages

- [[FAQ]] — Frequently asked questions
- [[Tested Hardware]] — Community-reported working hardware
- [[PAM Reference]] — Additional PAM configuration notes

## Quick Links

- [Howdy Project](https://github.com/boltgolt/howdy) — The upstream face recognition project
- [dlib](http://dlib.net/) — Machine learning library used for face recognition
- [Fedora Documentation](https://docs.fedoraproject.org/) — Official Fedora docs

## Getting Help

1. Run the diagnostic: `sudo ./install-howdy.sh --diagnose`
2. Check the [[FAQ]] for common issues
3. Review the troubleshooting section in the [README](../blob/main/README.md#troubleshooting)
4. Open an issue with diagnostic output if the problem persists

## Contributing

- Report issues with your Fedora version and hardware details
- Submit pull requests for fixes or improvements
- Add your hardware to the [[Tested Hardware]] page
