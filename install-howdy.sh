...
# Called by pam_exec.so. Prints the scan result to stdout (relayed to the
# calling application by pam_exec.so's `stdout` flag, so sudo/GDM/polkit
# display it inline) and also writes to the system journal. Exit codes are
# normalised to 0 (success) or 1 (failure) so pam_exec.so always gets a clean
# result regardless of compare.py's internal codes (10=no model, 11=timeout,
# 13=too dark, etc.).
#
...
CONFIG=/usr/lib64/security/howdy/config.ini
